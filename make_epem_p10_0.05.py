...
for positron_polarization in 'uxyzl':
    epem_card = WZDCard.WZDCard(f'cards/epem_{positron_polarization}10.sin')
    args = []
    for theta_p, e_com_p in zip(theta_ps, e_com_ps):
        print(f'run/epem_{positron_polarization}10_{theta_p:.4f}rad')
        if os.path.exists(f'run/epem_{positron_polarization}10_{theta_p:.4f}rad'): continue
        args.append({
            'workdir': f'run/epem_{positron_polarization}10_{theta_p:.4f}rad',
