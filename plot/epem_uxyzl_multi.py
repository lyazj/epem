...
for axis, (P3, P4, W) in zip('uxyzl', events):
    ...
    cos_theta_7 = np.cos(theta_7)
    if axis == 'z': axis = 'r'
    plt.hist(cos_theta_7, bins=50, weights=W, density=True, histtype='step', label=f'{axis} polarized')
plt.xlabel(r'$\cos\theta^\prime_7$')
plt.ylabel(r'$\frac{1}{\sigma}\frac{\mathrm{d}\sigma}{\mathrm{d}\cos\theta^\prime_7}$')
plt.yscale('log')
plt.legend()
plt.grid()
plt.tight_layout()
...
for axis, (P3, P4, W) in zip('uxyzl', events):
    ...
    if axis == 'z': axis = 'r'
    plt.hist(phi_7, bins=10, weights=W, density=True, histtype='step', label=f'{axis} polarized')
plt.xlabel(r'$\phi_7$ [rad]')
plt.ylabel(r'$\frac{1}{\sigma}\frac{\mathrm{d}\sigma}{\mathrm{d}\phi_7}\ [\mathrm{rad}^{-1}]$')
plt.yscale('log')
plt.legend()
plt.grid()
plt.tight_layout()
